...
## Status: Phase 3 COMPLETE · Next: Phase 4 — cache.py (flat-file LRU, byte-cap invariant, atomic writes, reconcile, ENOSPC degrade, startup scan)
...
- Suite: **295 unit tests green** (`uv run pytest -m "not integration"` → "295 passed, 1 deselected").
...
## NEXT UNIT → Phase 4: `ytt/cache.py` (flat-file LRU, byte-cap invariant, atomic writes, reconcile, ENOSPC degrade, startup scan)

## cache.py plan (Phase 4)

`ytt/cache.py` implements the flat-file transcript cache (plan §Caching):

**Files:** `<video_id>.<lang>.txt` (text body) + `<video_id>.<lang>.json` (segments + metadata sidecar). Caption unit uses served lang; Whisper unit uses `whisper` as lang key.

**Key operations:**
- `get(video_id, lang) -> CacheHit | None` — check both txt+json; touch mtime on hit; check whisper fallback for caption miss
- `put(video_id, lang, text, segments, source, metadata) -> None` — atomic write (`<file>.tmp` → `os.replace`), increment byte counter, evict LRU if over cap
- `startup_scan()` — scan dir, build in-memory byte total, clean stray `.tmp` files
- `reconcile()` — re-stat all units, recompute total, evict if over cap; log drift
- `evict_lru(needed_bytes) -> int` — evict oldest-mtime units until `needed_bytes` fits; whole units only; under cache lock
- ENOSPC: evict-and-retry once, then serve-but-don't-cache (degrade, not error)
- All mtime touch + eviction + delete under one asyncio lock (Invariant 1)
- Invariant 1 property-based test: after N concurrent writes, total_bytes <= cache_max_bytes

**Cache unit constants:**
- `<id>.<lang>.txt` + `<id>.<lang>.json` = one atomic unit
- Whisper: `<id>.whisper.txt` + `<id>.whisper.json`
- `.tmp` files cleaned on startup scan
