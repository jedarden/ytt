...
7. **Egress residential** — `/admin/egress`/canary reports non-datacenter ASN; fetch succeeds without a proxy. **Satisfied 2026-09-18, outcome `ok`** (bead ytt-590de689): canary pod `ytt-canary-7898978459-mw7l4` completed 3/3 caption fetches from-cluster (22:54:00/23:04:02/23:14:05 UTC, `ytt_canary_failures_total=0`); evidence in `docs/notes/canary-first-fetch.md`.
...
|---|---|---|---|
| Residential egress is "decisive, free" (HIGH) — **PROVEN 2026-09-18** (first positive in-cluster fetch; bead ytt-590de689) | ardenone-cluster egresses a non-datacenter ASN at deploy: startup probe `is_residential=true` (AS701 Verizon), **and** canary pod `ytt-canary-7898978459-mw7l4` (node k3s-agent-minisforum, image 0.2.20) completed 3/3 yt-dlp caption fetches against YouTube — 2026-09-18 22:54:00/23:04:02/23:14:05 UTC at the 600s cadence, `ytt_canary_failures_total=0`, `ytt_fetch_empty_body_total=0`, last-success gauge matching the logs to the second (outcome `ok`). Evidence: `docs/notes/canary-first-fetch.md` | `egress_is_residential=0`; block-rate spike; canary empty-body — the falsifier stays live: `ytt_canary_failures_total` rising or a stale `ytt_canary_last_success_timestamp_seconds` (`YttCanaryFailed` alert) | Set `YTT_PROXY_URL` (Webshare) — **but Webshare is datacenter-range residential proxy you pay for; the "free" premise collapses and the cost/risk calculus changes.** Re-evaluate hosting. |
...
## Status & bead tracking

This plan deliberately carries **no bead-status section**. A bead list pasted into a plan is stale the moment it is committed — the committed 2026-09-17 inventory snapshot was a week old when a planning strand consulted it, and every "not-closed" bead it listed had closed in the meantime (bead `ytt-042fee08`). Status has exactly two homes:

- **Authoritative: the live bead-rs store.** Run `bead list` (or `bead list --ready`) in this workspace. Nothing committed to the repo overrides it; cite its output, not any file, when the current count matters.
- **Snapshot: `docs/bead-inventory.md` + `docs/bead-inventory.json`** — fully generated by `scripts/regen-bead-inventory.sh`, committed point-in-time copies with an explicit caveat. Never hand-edit either; regenerate and commit when a citable snapshot is wanted.

`tests/unit/test_bead_inventory_docs.py` keeps it that way: the generated pair must agree with each other, curated docs must carry no hand-written bead-status claims (that lint is what stands in for a status section here), and where a live bead store exists the committed snapshot must be at most 14 days old — regenerate and commit when that gate trips.
...
> it via the existing `docker-hub-registry` pull secret, and no GHCR push
> credential was ever provisioned (the box's GitHub token has no
...
`jedarden/ytt` is **public**, so the image must live where the community can pull it. Decision (as planned at Phase 11 — **superseded by the addendum above for the registry choice**):
